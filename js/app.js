...
const RIVERWATCH_SCENE_STATES = ['TAILWIND','CALM','HEADWIND','ROUGH','STORM'];
const RIVERWATCH_SCENE_TRENDS = ['STRONG_DOWN','DOWN','STABLE','UP','STRONG_UP'];

function getIntroRoundRobinStateTrend(date = new Date()) {
    const day = Number(date.getDate()) || 1;
    const index = (day - 1) % 25;
    return {
        state: RIVERWATCH_SCENE_STATES[Math.floor(index / 5)],
        trend: RIVERWATCH_SCENE_TRENDS[index % 5],
        index
    };
}

function applyIntroSceneConfig() {
    const cfg = getMobileHeaderConfig();
    const scene = document.getElementById('rwIntroScene');
    if (!cfg || !scene) return;

    const setPlace = (selector, c, baseWidthRatio) => {
    ...
    setPlace('.rw-intro-boat-place', cfg.boat, cfg.boat?.baseWidthRatio ?? .37);
    setPlace('.rw-intro-front-place', cfg.frontWave, cfg.frontWave?.baseWidthRatio ?? 1);
    setPlace('.rw-intro-rear-place', cfg.rearWave, cfg.rearWave?.baseWidthRatio ?? 1);

    const boatMotion = scene.querySelector('.rw-intro-boat-motion');
    if (boatMotion && cfg.boat) {
        boatMotion.style.transformOrigin = `${cfg.boat.originX ?? 50}% ${cfg.boat.originY ?? 82}%`;
        const a = cfg.boat.animation || {};
        boatMotion.style.setProperty('--rw-drift-x', `${a.driftX ?? 5}px`);
    ...
    const applyWaveMotion = (selector, c) => {
        const el = scene.querySelector(selector);
        if (!el || !c) return;
        const a = c.animation || {};
        el.style.setProperty('--rw-drift-x', `${a.driftX ?? 0}px`);
        el.style.setProperty('--rw-float-y', `${a.floatY ?? 0}px`);
        el.style.setProperty('--rw-scale-pulse', String(a.scalePulse ?? 0));
        el.style.animationDuration = `${a.duration ?? 5}s`;
        el.style.animationTimingFunction = a.easing || 'ease-in-out';
    };
    applyWaveMotion('.rw-intro-front-motion', cfg.frontWave);
    applyWaveMotion('.rw-intro-rear-motion', cfg.rearWave);

    const layerMap = {
        darkCloud: '.rw-intro-darkcloud-layer',
        rain: '.rw-intro-rain-layer',
        lightning: '.rw-intro-lightning-layer',
        cloud: '.rw-intro-cloud-layer',
        birds: '.rw-intro-birds-layer'
    };
    Object.entries(cfg.trend?.layers || {}).forEach(([key, c]) => {
        const layer = scene.querySelector(layerMap[key]);
        if (!layer) return;
        layer.style.zIndex = String(c.zIndex);
        const place = layer.querySelector('.rw-intro-trend-place');
        if (place) {
            place.style.right = '0';
            place.style.left = 'auto';
            place.style.top = '0';
            place.style.transformOrigin = '100% 0';
            place.style.transform = `translate3d(${-c.x}px,${c.y}px,0) scale(${c.scale})`;
        }
        const img = layer.querySelector('img');
        if (img) img.style.opacity = String(c.opacity);
        const motion = layer.querySelector('[class$="-motion"]');
        if (motion) {
            motion.style.setProperty('--rw-drift-x', `${c.driftX ?? 0}px`);
            motion.style.setProperty('--rw-float-y', `${c.floatY ?? 0}px`);
            motion.style.animationDuration = `${c.duration ?? 5}s`;
        }
    });
}

function renderIntroSceneRoundRobin(date = new Date()) {
    const scene = document.getElementById('rwIntroScene');
    const boat = document.getElementById('rwIntroBoat');
    const front = document.getElementById('rwIntroFrontWave');
    if (!scene || !boat || !front) return;

    applyIntroSceneConfig();
    const { state, trend, index } = getIntroRoundRobinStateTrend(date);
    scene.dataset.state = state;
    scene.dataset.trend = trend;
    scene.dataset.roundRobinIndex = String(index + 1);
    ...
    const composition = getMobileHeaderConfig()?.trend?.composition || {
        ...
        STABLE:['darkCloud'],
        UP:['cloud'],
        STRONG_UP:['cloud','birds']
    };
    const active = new Set(composition[trend] || []);
    const layers = {
        darkCloud: scene.querySelector('.rw-intro-darkcloud-layer'),
        rain: scene.querySelector('.rw-intro-rain-layer'),
        lightning: scene.querySelector('.rw-intro-lightning-layer'),
        cloud: scene.querySelector('.rw-intro-cloud-layer'),
        birds: scene.querySelector('.rw-intro-birds-layer')
    };
    Object.entries(layers).forEach(([key, el]) => {
        if (el) el.style.display = active.has(key) ? 'block' : 'none';
    });
}
...
window.addEventListener("DOMContentLoaded", () => {
    // Intro scene is date-deterministic and renders before remote state/trend data is loaded.
    renderIntroSceneRoundRobin();
